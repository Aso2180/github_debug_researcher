  ...
  components: [
    {
      ...
      description: 'フロントエンド開発で最も広く使われるライブラリ。',
...
test('Step1→2→3と進み、コンポーネント・リスク文言・実データが表示される', async () => {
  ...
  // 数値の意味・コンポーネントの用途が分かりにくいというフィードバックへの対応(スコア解説・凡例・
  // コンポーネント説明・上位/下位ラベル)
  expect(screen.getByText('フロントエンド開発で最も広く使われるライブラリ。')).toBeInTheDocument();
  expect(screen.getByText(/総合リスクスコア/)).toBeInTheDocument();
  expect(screen.getByText('実データ: 該当リポジトリ(リスク上位)')).toBeInTheDocument();
  expect(screen.getByText('実データ: 該当リポジトリ(リスク下位)')).toBeInTheDocument();
});
