...
test('GET /api/architecture-patterns/:slug はパターン全体のmatchedLanguagesを返す(ジャーニー画面のハイライト用)', async () => {
  const res = await get('/api/architecture-patterns/efficiency-enterprise');
  const body = await res.json();
  // "React/TypeScript"はTypeScriptトークンが一致するが、"Docker"は収集対象言語に存在しないため一致しない
  assert.deepEqual(body.matchedLanguages, ['TypeScript']);
});
